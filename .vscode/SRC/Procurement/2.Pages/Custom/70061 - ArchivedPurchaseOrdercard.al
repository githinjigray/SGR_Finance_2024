page 70061 "Purchase Order-Archived"
{
    Caption = 'Purchase Order- Archived';
    Editable = false;
    DeleteAllowed = false;
    PageType = Document;
                    ...
                    field(PayToOptions; PayToOptions)
                    {
                        ...
                        begin
                            if PayToOptions = PayToOptions::"Default (Vendor)" then
                                Rec.Validate(Rec."Pay-to Vendor No.", Rec."Buy-from Vendor No.");
                        end;
                    }
                    group(Control95)
                    {
                        ...
                        field("Pay-to Name"; Rec."Pay-to Name")
                        {
                            ...
                            begin
                                if not ((PayToOptions = PayToOptions::"Custom Address") and not ShouldSearchForVendByName) then begin
                                    if Rec.GetFilter(Rec."Pay-to Vendor No.") = xRec."Pay-to Vendor No." then
                                        if Rec."Pay-to Vendor No." <> xRec."Pay-to Vendor No." then
                                            Rec.SetRange(Rec."Pay-to Vendor No.");
                        ...
                        field("Pay-to Address"; Rec."Pay-to Address")
                        {
                            ApplicationArea = Basic, Suite;
                            Caption = 'Address';
                            // Editable = (Rec.PayToOptions = Rec.PayToOptions::"Custom Address") OR (Rec."Buy-from Vendor No." <> Rec."Pay-to Vendor No.");
                            // Enabled = (Rec.PayToOptions = Rec.PayToOptions::"Custom Address") OR (Rec."Buy-from Vendor No." <> Rec."Pay-to Vendor No.");
                            Importance = Additional;
                            QuickEntry = false;
                        ...
                        group(Control123)
                        {
                            ...
                            field("Pay-to County"; Rec."Pay-to County")
                            {
                                ApplicationArea = Basic, Suite;
                                Caption = 'County';
                                // Editable = (Rec.PayToOptions = Rec.PayToOptions::"Custom Address") OR ("Buy-from Vendor No." <> "Pay-to Vendor No.");
                                // Enabled = (Rec.PayToOptions = Rec.PayToOptions::"Custom Address") OR ("Buy-from Vendor No." <> "Pay-to Vendor No.");
                                Importance = Additional;
                                QuickEntry = false;
                                ToolTip = 'Specifies the state, province or county of the address.';
                            }
    ...
    begin
        case ShipToOptions of
            ...
            ShipToOptions::"Custom Address":
                begin
                    Rec.Validate(Rec."Sell-to Customer No.", '');
